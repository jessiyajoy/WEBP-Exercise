<?php
    ...
    $is_completed = check_completion($con, $curr_course, $user_data['user_id']);
    ...
    function doEnrolling($con, $user_data, $table) {
        ...
        if (mysqli_query($con, $sql)) {
            $out = $out . '<script>window.location.href = "successCourseEnroll.php?course=HTML&gotopage=coursepage-html.php";</script>';
            $status = TRUE;
        } else {
            $STATUS = FALSE;
    ...
    function doReEnrolling($con, $user_data, $table) {
        doUnEnrolling($con, $user_data, $table);
        return doReEnrolling($con, $user_data, $table);
    }

    function doCompletion($con, $user_data, $table) {
        $user_id_submit = $user_data['user_id'];
        $user_id_submit = $user_data['user_id'];
        $user_name_submit = $user_data['user_name'];
                    ...
                    ?>>
                        <?php 
                            if(isset($_POST['Completion'])) {
                                echo doCompletion($con, $user_data, "EnrolledStudentsHTML");
                            } else {
